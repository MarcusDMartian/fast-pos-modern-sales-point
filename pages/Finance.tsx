...
const Finance: React.FC = () => {
  const { expenses, addExpense, currentUser, showToast, enterpriseConfig } = useStore();
  ...
  return (
    <div className="p-3 md:p-8">
      ...
      <div className="flex items-center justify-between gap-4 mb-6 md:mb-10">
        <div>
          <h1 className="text-xl md:text-4xl font-extrabold text-slate-800 tracking-tight">Tài Chính</h1>
          <p className="hidden md:block text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] mt-2">Quản lý ngân sách, thu chi và sổ kế toán</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowAddVoucher(true)}
            className="p-3 md:px-6 md:py-4 bg-emerald-500 text-white font-bold rounded-xl md:rounded-2xl flex items-center gap-2 shadow-lg hover:bg-emerald-600 transition-all text-sm"
          >
            <ArrowUpCircle size={18} />
            <span className="hidden md:inline">Phiếu Thu</span>
          </button>
          <button
            onClick={() => setShowAddExpense(true)}
            className="p-3 md:px-6 md:py-4 bg-orange-500 text-white font-bold rounded-xl md:rounded-2xl flex items-center gap-2 shadow-lg hover:bg-orange-600 transition-all text-sm"
          >
            <ArrowDownCircle size={18} />
            <span className="hidden md:inline">Phiếu Chi</span>
          </button>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex items-center gap-2 mb-4 md:mb-8 overflow-x-auto no-scrollbar">
        {[
          ...
          <button
            ...
            className={`shrink-0 px-3 md:px-5 py-2 md:py-3 rounded-xl md:rounded-2xl font-bold text-xs md:text-sm flex items-center gap-1.5 md:gap-2 transition-all whitespace-nowrap ${activeTab === tab.id
      ...
      {activeTab === 'overview' && (
        <div className="space-y-8 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
            <div className="glass-card p-4 md:p-6 hover:scale-102 transition-transform">
              <div className="p-2 md:p-3 bg-emerald-50 text-emerald-500 rounded-lg md:rounded-xl w-fit mb-2 md:mb-4"><ArrowUpCircle size={20} className="md:w-6 md:h-6" /></div>
              <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wide">Tổng thu</p>
              <h3 className="text-lg md:text-2xl font-black text-emerald-500">{formatCurrency(totalReceiptVouchers, enterpriseConfig.currency)}</h3>
            </div>
            <div className="glass-card p-4 md:p-6 hover:scale-102 transition-transform">
              <div className="p-2 md:p-3 bg-orange-50 text-orange-500 rounded-lg md:rounded-xl w-fit mb-2 md:mb-4"><ArrowDownCircle size={20} className="md:w-6 md:h-6" /></div>
              <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wide">Tổng chi</p>
              <h3 className="text-lg md:text-2xl font-black text-orange-500">{formatCurrency((totalPaymentVouchers + totalExpense), enterpriseConfig.currency)}</h3>
            </div>
            <div className="glass-card p-4 md:p-6 hover:scale-102 transition-transform">
              <div className="p-2 md:p-3 bg-amber-50 text-amber-500 rounded-lg md:rounded-xl w-fit mb-2 md:mb-4"><Clock size={20} className="md:w-6 md:h-6" /></div>
              <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wide">Chờ duyệt</p>
              <h3 className="text-lg md:text-2xl font-black text-amber-500">{pendingVouchers.length}</h3>
            </div>
            <div className="bg-slate-900 p-4 md:p-6 rounded-xl md:rounded-2xl text-white hover:scale-102 transition-transform">
              <div className="p-2 md:p-3 bg-white/10 text-primary rounded-lg md:rounded-xl w-fit mb-2 md:mb-4"><Wallet size={20} className="md:w-6 md:h-6" /></div>
              <p className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wide">Tiền mặt ròng</p>
              <h3 className="text-lg md:text-2xl font-black">{formatCurrency((totalReceiptVouchers - totalPaymentVouchers - totalExpense), enterpriseConfig.currency)}</h3>
            </div>
          </div>
                ...
                <div key={budget.id} className="p-4 bg-white/50 rounded-2xl">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-right">
                      <p className="font-black text-slate-800">{formatCurrency(budget.spent, enterpriseConfig.currency)}</p>
                      <p className="text-[10px] text-slate-400">/ {formatCurrency(budget.allocated, enterpriseConfig.currency)}</p>
                    </div>
                  </div>
          ...
          <div className="glass-card p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="p-6 bg-slate-50 rounded-2xl text-center">
                ...
                <p className="font-black text-2xl text-slate-800">{formatCurrency(totalBudgetAllocated, enterpriseConfig.currency)}</p>
              </div>
              <div className="p-6 bg-emerald-50 rounded-2xl text-center">
                ...
                <p className="font-black text-2xl text-emerald-500">{formatCurrency(totalBudgetSpent, enterpriseConfig.currency)}</p>
              </div>
              <div className="p-6 bg-blue-50 rounded-2xl text-center">
                ...
                <p className="font-black text-2xl text-blue-500">{formatCurrency((totalBudgetAllocated - totalBudgetSpent), enterpriseConfig.currency)}</p>
              </div>
                ...
                <tbody>
                  {budgets.map(budget => (
                    <tr key={budget.id} className="border-b border-slate-50 hover:bg-white/50 transition-colors">
                      ...
                      <td className="p-4 text-right font-bold text-slate-600">{formatCurrency(budget.allocated, enterpriseConfig.currency)}</td>
                      <td className="p-4 text-right font-bold text-orange-500">{formatCurrency(budget.spent, enterpriseConfig.currency)}</td>
                      <td className="p-4 text-right font-bold text-emerald-500">{formatCurrency((budget.allocated - budget.spent), enterpriseConfig.currency)}</td>
                      <td className="p-4">
            ...
            <div className="md:hidden p-4 space-y-3">
              {budgets.map(budget => (
                <div key={budget.id} className="bg-white rounded-xl p-4 border border-slate-100">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-sm text-slate-800">{budget.name}</span>
                    <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[8px] font-bold uppercase">{budget.category}</span>
                  </div>
                  <div className="border-t border-slate-100 mt-3 pt-3 grid grid-cols-3 gap-2 text-xs">
                    <div className="text-center">
                      <p className="text-slate-400 text-[9px]">Phân bổ</p>
                      <p className="font-bold text-slate-600">{formatCurrency(budget.allocated, enterpriseConfig.currency)}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-slate-400 text-[9px]">Đã chi</p>
                      <p className="font-bold text-orange-500">{formatCurrency(budget.spent, enterpriseConfig.currency)}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-slate-400 text-[9px]">Còn lại</p>
                      <p className="font-bold text-emerald-500">{formatCurrency((budget.allocated - budget.spent), enterpriseConfig.currency)}</p>
                    </div>
                  </div>
                  <div className="mt-3">
                    <div className="w-full h-2 bg-slate-100 rounded-full">
                    ...
                    <p className="text-[10px] text-slate-400 text-right mt-1">{((budget.spent / budget.allocated) * 100).toFixed(0)}%</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === 'expenses' && (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="glass-card overflow-hidden">
                ...
                <tbody>
                  {expenses.length > 0 ? expenses.map(exp => (
                    <tr key={exp.id} className="border-b border-slate-50 hover:bg-white/50 transition-colors">
                      ...
                      <td className="p-4 pr-6 text-right font-black text-orange-500 text-lg">-{formatCurrency(exp.amount, enterpriseConfig.currency)}</td>
                    </tr>
                  )) : (
                    <tr>
            ...
            <div className="md:hidden p-4 space-y-3">
              {expenses.length > 0 ? expenses.map(exp => (
                <div key={exp.id} className="bg-white rounded-xl p-4 border border-slate-100">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-bold text-sm text-slate-800">{exp.id}</span>
                      <p className="text-[10px] text-slate-400">{new Date(exp.date).toLocaleDateString('vi-VN')}</p>
                    </div>
                    <span className="px-2 py-0.5 bg-orange-50 text-orange-500 rounded text-[8px] font-bold uppercase">{exp.category}</span>
                  </div>
                  <div className="border-t border-slate-100 mt-3 pt-3 space-y-2">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Người tạo</span>
                      <span className="font-medium text-slate-600">{exp.createdBy}</span>
                    </div>
                    {exp.note && (
                      <div className="flex justify-between text-xs">
                        <span className="text-slate-400">Ghi chú</span>
                        <span className="font-medium text-slate-600 truncate max-w-[150px]">{exp.note}</span>
                      </div>
                    )}
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400 text-xs">Số tiền</span>
                      <span className="font-black text-orange-500 text-base">-{formatCurrency(exp.amount, enterpriseConfig.currency)}</span>
                    </div>
                  </div>
                </div>
              )) : (
                <div className="p-12 text-center text-slate-400">
                  <ArrowDownCircle size={32} className="mx-auto mb-3 opacity-30" />
                  <p className="font-bold text-xs">Chưa có khoản chi</p>
                </div>
              )}
            </div>
          </div>
                  ...
                  <div key={voucher.id} className="p-4 bg-white/50 rounded-2xl flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <p className={`font-black text-lg ${voucher.type === 'receipt' ? 'text-emerald-500' : 'text-orange-500'}`}>
                        {voucher.type === 'receipt' ? '+' : '-'}{formatCurrency(voucher.amount, enterpriseConfig.currency)}
                      </p>
                      <button
                  ...
                  {vouchers.map(voucher => (
                    <tr key={voucher.id} className="border-b border-slate-50 hover:bg-white/50 transition-colors">
                      ...
                      <td className="p-4 text-center">
                        <span className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase ${voucher.status === 'approved' ? 'bg-emerald-50 text-emerald-500' : voucher.status === 'rejected' ? 'bg-red-50 text-red-500' : 'bg-amber-50 text-amber-500'}`}>
            ...
            <div className="md:hidden p-4 space-y-3">
              {vouchers.map(voucher => (
                <div key={voucher.id} className="bg-white rounded-xl p-4 border border-slate-100">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-sm text-slate-800">{voucher.id}</span>
                      <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${voucher.type === 'receipt' ? 'bg-emerald-50 text-emerald-500' : 'bg-orange-50 text-orange-500'}`}>
                        {voucher.type === 'receipt' ? 'Thu' : 'Chi'}
                      </span>
                    </div>
                    <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${voucher.status === 'approved' ? 'bg-emerald-50 text-emerald-500' : voucher.status === 'rejected' ? 'bg-red-50 text-red-500' : 'bg-amber-50 text-amber-500'}`}>
                      {voucher.status === 'approved' ? 'Duyệt' : voucher.status === 'rejected' ? 'Từ chối' : 'Chờ'}
                    </span>
                  </div>
                  <div className="border-t border-slate-100 mt-3 pt-3 space-y-2">
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Diễn giải</span>
                      <span className="font-medium text-slate-600 truncate max-w-[150px]">{voucher.description}</span>
                    </div>
                    <div className="flex justify-between text-xs">
                      <span className="text-slate-400">Người tạo</span>
                      <span className="font-medium text-slate-600">{voucher.createdBy}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400 text-xs">Số tiền</span>
                      <span className={`font-black text-base ${voucher.type === 'receipt' ? 'text-emerald-500' : 'text-orange-500'}`}>
                        {voucher.type === 'receipt' ? '+' : '-'}{formatCurrency(voucher.amount, enterpriseConfig.currency)}
                      </span>
                    </div>
                  </div>
              ...
              <table className="w-full min-w-[600px]">
                ...
                <tbody className="text-slate-300">
                  <tr className="border-b border-white/5">
                    ...
                    <td className="py-3 text-right font-bold">{formatCurrency(5000000, enterpriseConfig.currency)}</td>
                  </tr>
                  <tr className="border-b border-white/5">
                    <td className="py-3">31/12</td>
                    ...
                    <td className="py-3 text-emerald-400">111 - Tiền mặt</td>
                    <td className="py-3 text-right font-bold">{formatCurrency(2500000, enterpriseConfig.currency)}</td>
                  </tr>
                  <tr className="border-b border-white/5">
                    ...
                    <td className="py-3 text-emerald-400">111 - Tiền mặt</td>
                    <td className="py-3 text-right font-bold">{formatCurrency(800000, enterpriseConfig.currency)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-xl p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl animate-in zoom-in-95">
            ...
            <div className="space-y-6">
              <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Số tiền ({getCurrencySymbol(enterpriseConfig.currency)})</label>
                <input
                  autoFocus
                  type="number"
                  placeholder="0"
