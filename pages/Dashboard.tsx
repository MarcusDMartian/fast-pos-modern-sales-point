...
const Dashboard: React.FC = () => {
  ...
  const stats = [
    { label: 'Total Revenue', value: '$24,560', change: '+12.5%', icon: DollarSign, color: 'text-emerald-500', bg: 'bg-emerald-50' },
    { label: 'Total Orders', value: '1,240', change: '+5.2%', icon: ShoppingBag, color: 'text-primary', bg: 'bg-primary/10' },
    { label: 'Active Customers', value: '890', change: '+8.1%', icon: Users, color: 'text-blue-500', bg: 'bg-blue-50' },
    { label: 'Average Value', value: '$19.80', change: '-2.4%', icon: TrendingUp, color: 'text-orange-500', bg: 'bg-orange-50' },
  ];

  const recentActivities = [
    { id: 1, title: 'Sale Completed', subtitle: 'Order #9921', amount: '+$45.00', time: '2m ago', icon: CheckCircle2, color: 'text-emerald-500', bg: 'bg-emerald-50' },
    { id: 2, title: 'Stock Updated', subtitle: 'Matcha Latte +20', amount: 'Inventory', time: '15m ago', icon: Package, color: 'text-blue-500', bg: 'bg-blue-50' },
    { id: 3, title: 'Refund Issued', subtitle: 'Order #9812', amount: '-$12.50', time: '1h ago', icon: RefreshCcw, color: 'text-orange-500', bg: 'bg-orange-50' },
  ];
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-extrabold text-slate-900 mb-2 tracking-tight">Store Performance</h1>
          <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em]">Live business intelligence & operational insights</p>
        </div>
        <button
          onClick={handleExportReport}
          disabled={isExporting}
          className={`bg-primary text-white px-8 py-4 rounded-2xl font-bold flex items-center gap-3 shadow-xl shadow-primary-glow hover:bg-slate-900 transition-all text-[11px] uppercase tracking-wider scale-105 active:scale-95 ${isExporting ? 'opacity-70 grayscale' : ''}`}
        >
          {isExporting ? (
            <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <div key={idx} className="glass-card p-8 flex flex-col gap-5 hover:scale-105 transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div className={`p-4 rounded-2xl ${stat.bg} ${stat.color} shadow-inner`}>
                <stat.icon size={24} />
              </div>
              <div className={`text-[10px] font-bold px-3 py-1 rounded-lg ${stat.change.startsWith('+') ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                {stat.change}
              </div>
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="colorSales" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#F57255" stopOpacity={0.2} />
                    <stop offset="95%" stopColor="#F57255" stopOpacity={0} />
                  </linearGradient>
                </defs>
        ...
        <div className="glass-card p-8 flex flex-col">
          <div>
            <h3 className="text-xl font-extrabold text-slate-800 tracking-tight">Recent Activity</h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 mb-8">Operational log</p>
          </div>
