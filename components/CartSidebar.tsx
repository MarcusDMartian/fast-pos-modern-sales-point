...
const CartSidebar: React.FC<CartSidebarProps> = ({
  ...
  return (
    <>
      ...
      <div className={`glass-surface fixed right-3 top-[6.5rem] bottom-3 flex z-[110] transition-all duration-500 ease-in-out overflow-hidden rounded-[1.5rem] ${isCheckoutExpanded ? 'w-[calc(100vw-3rem)] lg:w-[1200px]' : 'w-full lg:w-[420px]'
        }`}>
          ...
          <div className="p-6 border-b border-white/40 bg-white/30">
            <div className="flex items-center justify-between mb-3">
              <div className="flex flex-col">
                <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Current Order</h2>
              ...
              <div className="flex gap-2">
                <button onClick={onSaveDraft} className="p-3 bg-white/40 hover:bg-white text-slate-600 rounded-xl transition-all border border-white/60 shadow-sm" title="Save Draft">
                  <Save size={18} />
                </button>
                <button onClick={onSplitOrder} className="p-3 bg-white/40 hover:bg-white text-slate-600 rounded-xl transition-all border border-white/60 shadow-sm" title="Split Order">
                  <Scissors size={18} />
                </button>
              </div>
            </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <p className="text-primary font-bold text-xs">
                          ${((item.totalPrice * item.quantity) - (item.discount ? (item.discount.type === 'percentage' ? (item.totalPrice * item.quantity * item.discount.value / 100) : item.discount.value) : 0)).toFixed(2)}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="p-6 bg-white border-t border-slate-100 rounded-t-[2rem] shadow-2xl space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <span className="text-slate-800 font-black">{totalQuantity} món</span>
              </div>
              <div className="flex justify-between text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                <span>Subtotal</span>
              ...
              <div className="flex justify-between pt-3 border-t border-slate-100 mt-1">
                <span className="text-xs font-bold text-slate-900 uppercase tracking-tight">Total Due</span>
                <span className="text-2xl font-extrabold text-primary tracking-tighter">${total.toFixed(2)}</span>
              </div>
            </div>
            ...
            {!isCheckoutExpanded ? (
              ...
              <div className="py-4 text-center border-2 border-dashed border-blue-100 rounded-2xl bg-blue-50/30">
                <p className="text-[10px] font-bold text-primary animate-pulse uppercase tracking-widest">Processing Payment...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/30 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-xl font-black text-slate-900">Search Customer</h2>
            ...
            <div className="p-6 space-y-6">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 pl-12 pr-4 py-4 rounded-2xl text-sm font-bold text-[#333984] outline-none border-2 border-transparent focus:border-[#0062FF] transition-all"
                />
              </div>
              ...
              <div className="max-h-[40vh] overflow-y-auto custom-scrollbar space-y-3">
                {filteredCustomers.length > 0 ? (
                  filteredCustomers.map(cust => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-[#333984] font-black">
                          {cust.name.charAt(0)}
                        </div>
                        <div>
                  ...
                  <div className="py-12 text-center opacity-30 italic text-xs font-black uppercase tracking-widest flex flex-col items-center">
                    ...
                    {customerSearch ? "No customer found" : "Search to start"}
                  </div>
                )}
              </div>
            </div>
