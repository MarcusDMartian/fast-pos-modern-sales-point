      ...
      <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 flex flex-col h-[85vh]">
        <div className="p-8 border-b border-gray-50 bg-white relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h2 className="text-2xl font-black text-[#333984]">Reserve Table {table.number}</h2>
              <p className="text-gray-500 font-bold text-xs uppercase tracking-widest mt-1">
                {step === 'customer' ? 'Guest Info' : step === 'menu' ? 'Pre-order Menu' : 'Time & Optimization'}
              </p>
            </div>
            ...
            {(['customer', 'menu', 'details'] as BookingStep[]).map((s, idx) => (
              <React.Fragment key={s}>
                <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                  step === s ? 'bg-[#2A46FF] scale-125' : idx < ['customer', 'menu', 'details'].indexOf(step) ? 'bg-green-400' : 'bg-gray-200'
                }`} />
                ...
                {filteredCustomers.map(cust => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-[#333984] font-black">{cust.name.charAt(0)}</div>
                      <div className="text-left">
                        <p className="font-black text-[#333984] text-lg">{cust.name}</p>
                        <p className="text-xs font-bold text-gray-400">{cust.phone}</p>
                      </div>
                    </div>
                    <ChevronRight size={20} className="text-gray-300 group-hover:text-[#2A46FF] transition-colors" />
                  </button>
                ))}
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right duration-300">
               <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 pl-14 pr-6 py-4 rounded-[1.5rem] text-sm font-bold text-[#333984] outline-none"
                />
              </div>
                    ...
                    return (
                      <div key={product.id} className="p-4 bg-gray-50 rounded-3xl flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                           <p className="text-[10px] font-black text-[#333984] truncate">{product.name}</p>
                           <p className="text-[10px] font-bold text-[#2A46FF]">${product.price}</p>
                        </div>
                        <div className="flex items-center gap-2">
                           {inCart ? (
                             ...
                             <button onClick={() => toggleItem(product, 1)} className="p-1.5 bg-white text-[#2A46FF] rounded-lg shadow-sm"><Plus size={14}/></button>
                           )}
                        </div>
                      </div>
                    );
              ...
              {preOrderItems.length > 0 && (
                <div className="p-6 bg-[#333984] rounded-[2rem] text-white">
                   ...
                   <div className="space-y-2">
                      {preOrderItems.map(item => (
                        <div key={item.id} className="flex justify-between items-center py-2 border-b border-white/10">
                           ...
                           <span className="text-xs font-black">${(item.totalPrice * item.quantity).toFixed(2)}</span>
                        </div>
                      ))}
                   </div>
                </div>
            ...
            <div className="space-y-8 animate-in zoom-in duration-300">
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                       ...
                       <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full bg-gray-50 pl-16 pr-6 py-6 rounded-[2rem] text-lg font-black text-[#333984] outline-none" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-2">Giờ bắt đầu</label>
                    <div className="relative">
                       <Clock className="absolute left-6 top-1/2 -translate-y-1/2 text-[#2A46FF]" size={20}/>
                       <input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="w-full bg-gray-50 pl-16 pr-6 py-6 rounded-[2rem] text-xl font-black text-[#333984] outline-none" />
                    </div>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                       <Users className="absolute left-6 top-1/2 -translate-y-1/2 text-[#2A46FF]" size={20}/>
                       <input type="number" value={guests} onChange={(e) => setGuests(parseInt(e.target.value))} className="w-full bg-gray-50 pl-16 pr-6 py-6 rounded-[2rem] text-xl font-black text-[#333984] outline-none" />
                    </div>
                  </div>
                  <div className="space-y-2">
                     ...
                     <div className="flex gap-2">
                        {[60, 120, 180].map(m => (
                          <button key={m} onClick={() => setDuration(m)} className={`flex-1 py-4 rounded-2xl font-black text-[10px] transition-all ${duration === m ? 'bg-[#333984] text-white shadow-lg' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}>{m}m</button>
                        ))}
                     </div>
                  </div>
               </div>
               ...
               <div className="space-y-2">
                  ...
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Yêu cầu đặc biệt..." className="w-full bg-gray-50 p-6 rounded-[2.5rem] text-sm font-bold text-[#333984] outline-none resize-none" rows={3} />
               </div>
        ...
        <div className="p-8 bg-[#F4F6FF]/50 border-t border-gray-50 flex gap-4">
           {step !== 'customer' && (
             <button onClick={() => setStep(step === 'details' ? 'menu' : 'customer')} className="p-6 bg-white border border-gray-200 text-[#333984] rounded-[2rem] hover:bg-gray-50 transition-all"><ChevronLeft/></button>
           )}
           <button 
