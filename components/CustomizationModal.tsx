...
const CustomizationModal: React.FC<CustomizationModalProps> = ({ product, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#333984]/30 backdrop-blur-md p-4">
      <div className="bg-white w-full max-w-xl rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        <div className="p-8 border-b border-gray-50 flex justify-between items-center">
          <h2 className="text-2xl font-black text-[#333984]">Customize Order</h2>
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
          <div className="flex gap-6">
            ...
            <div>
              <h3 className="text-xl font-bold text-[#333984]">{product.name}</h3>
              <p className="text-[#2A46FF] font-bold text-lg">${product.price.toFixed(2)} Base</p>
            </div>
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={`p-5 rounded-[1.5rem] border-2 transition-all flex justify-between items-center ${
                        isSelected ? 'border-[#2A46FF] bg-blue-50/50' : 'border-gray-50 bg-white'
                      }`}
                    >
                      <span className={`font-bold ${isSelected ? 'text-[#2A46FF]' : 'text-[#333984]'}`}>{val.name}</span>
                      <span className="text-xs font-bold text-gray-500">
                        {val.priceAdjustment > 0 ? `+$${val.priceAdjustment}` : 'Free'}
                      </span>
                    </button>
                  );
                })}
          ...
          <div className="space-y-4">
            <label className="text-sm font-black text-gray-500 uppercase tracking-widest">Special Note</label>
            <textarea
              placeholder="E.g. Less ice, no sugar, extra hot..."
              value={note}
              onChange={(e) => setNote(e.target.value)}
              className="w-full bg-gray-50 rounded-[2rem] p-6 text-[#333984] font-medium outline-none focus:ring-2 focus:ring-[#2A46FF]/10 transition-all resize-none"
              rows={3}
            />
          </div>
        </div>
        ...
        <div className="p-8 bg-[#F4F6FF]/50 border-t border-gray-50">
          <button
            ...
            className="w-full bg-[#2A46FF] text-white py-6 rounded-[2rem] font-bold text-lg flex items-center justify-center gap-3 shadow-xl shadow-blue-100 hover:bg-[#333984] transition-all"
          >
